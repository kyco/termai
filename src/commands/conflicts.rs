...
async fn analyze_conflicts(
    ...
    for (file, conflict_info) in &conflicts {
        ...
        println!(
            "   {} {}%",
            "Resolution confidence:".bright_white(),
...
async fn suggest_resolution_strategies(
    git_repo: &GitRepository,
    args: &crate::args::ConflictsArgs,
) -> Result<()> {
    println!(
    ...
    println!(
        "   {} {}",
        "Approach:".bright_white(),
        overall_strategy.approach.bright_yellow()
    );
    println!(
        "   {} {}",
        "Order:".bright_white(),
        overall_strategy.order_description
    );
    println!(
        "   {} {} minutes",
...
async fn interactive_conflict_resolution(
    git_repo: &GitRepository,
    args: &crate::args::ConflictsArgs,
) -> Result<()> {
    println!(
...
async fn show_conflict_status(_git_repo: &GitRepository) -> Result<()> {
    ...
    let progress_percent =
        (conflict_state.resolved_files as f64 / conflict_state.total_files as f64 * 100.0) as u32;
    println!(
        "   {} {}/{} files ({}/100)",
...
async fn show_resolution_guide(
    _git_repo: &GitRepository,
    _args: &crate::args::ConflictsArgs,
) -> Result<()> {
    ...
    println!(
        "   • {} - Get AI analysis",
        "termai conflicts analyze".green()
    );
    println!(
...
fn detect_conflicted_files(
    _status: &crate::git::repository::RepoStatus,
) -> Result<HashMap<String, ConflictInfo>> {
...
async fn analyze_file_conflicts(
    file: &str,
...
async fn show_quick_resolution_options(conflicts: &HashMap<String, ConflictInfo>) -> Result<()> {
    ...
    if conflicts.len() == 1 && conflicts.values().all(|c| c.markers.len() == 1) {
        println!(
            "\n   {} Simple conflict detected - quick resolution available!",
            "💡".green()
        );
    }
...
async fn generate_overall_strategy(
...
async fn generate_file_strategy(
    file: &str,
    conflict_info: &ConflictInfo,
    _args: &crate::args::ConflictsArgs,
) -> Result<FileStrategy> {
...
async fn show_conflict_preview(_file: &str, conflict_info: &ConflictInfo) -> Result<()> {
    ...
    for (i, marker) in conflict_info.markers.iter().enumerate() {
        ...
        println!(
            "   {} {}",
            "Ours:".green().bold(),
            marker.our_content.bright_white()
        );
        println!(
            "   {} {}",
            "Theirs:".red().bold(),
            marker.their_content.bright_white()
        );
    }
...
async fn resolve_with_editor(file: &str, _args: &crate::args::ConflictsArgs) -> Result<()> {
    ...
    println!(
        "   {} Please resolve conflicts manually and save the file",
        "💡".yellow()
    );
    Ok(())
}

async fn resolve_with_ai_suggestion(
    file: &str,
    conflict_info: &ConflictInfo,
    _args: &crate::args::ConflictsArgs,
) -> Result<()> {
    ...
    for marker in &conflict_info.markers {
        println!(
            "   {} Line {}: Consider combining both changes",
            "💡".bright_yellow(),
            marker.line_number.to_string().cyan()
        );

        let suggested_resolution = format!(
            "{} // Combined with: {}",
            marker.our_content, marker.their_content
        );

        println!(
            "   {} {}",
            "Suggestion:".bright_green(),
            suggested_resolution.white()
        );
    }
