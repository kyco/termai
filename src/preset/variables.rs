        ...
        for (name, var_def) in &template.variables {
            let value = match &var_def.var_type {
                ...
                _ => {
                    ...
                    let mut input = Input::<String>::new().with_prompt(&var_def.description);
                    ...
                    self.resolver
                        .resolve_variable(&var_def.var_type, &Value::String(result))?
                }
            };
...
mod tests {
    ...
    fn test_variable_resolver_string() {
        ...
        let result = resolver
            .resolve_variable(&VariableType::String, &Value::String("test".to_string()))
            .unwrap();
    ...
    fn test_variable_resolver_datetime() {
        let resolver = VariableResolver::new();
        let result = resolver
            .resolve_variable(&VariableType::DateTime, &Value::String("%Y".to_string()))
            .unwrap();
