...
impl PresetManager {
    ...
    pub fn new() -> Result<Self> {
        ...
        fs::create_dir_all(&user_presets_dir).context("Failed to create user presets directory")?;
    ...
    fn get_user_presets_dir() -> Result<PathBuf> {
        let config_dir =
            dirs::config_dir().context("Failed to get user configuration directory")?;
    ...
    pub fn use_preset_with_variables(
        ...
        let resolved_variables = self
            .collector
            .collect_from_values(&preset.template, variables)?;
    ...
    pub fn save_preset(&self, preset: &Preset) -> Result<()> {
        ...
        let content = serde_yaml::to_string(preset).context("Failed to serialize preset")?;
    ...
    pub fn delete_preset(&self, name: &str) -> Result<()> {
        ...
        fs::remove_file(&preset_info.path).with_context(|| {
            format!(
                "Failed to delete preset file: {}",
                preset_info.path.display()
            )
        })?;
