...
impl ChatFormatter {
    ...
    pub fn format_message(&self, role: &Role, content: &str, timestamp: Option<DateTime<Local>>) -> String {
    ...
    pub fn format_session_saved(&self, session_name: &str) -> String {
        format!("💾 Session saved as '{}'", session_name).bright_green().to_string()
    }
    
    /// Format conversation cleared message
    pub fn format_conversation_cleared(&self) -> String {
        "🗑️  Conversation history cleared".bright_yellow().to_string()
    }
}
